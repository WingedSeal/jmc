...
const CodeBlock: React.FC<Code> = (props) => {
    return <>
        <div className='relative my-2 md:my-4 bg-[#292D3E] rounded-md md:rounded-2xl text-[#eeffff] px-4 py-1 md:px-8 md:py-3'>
            <div className='m-0 p-0 pb-2 text-xs md:text-2xl overflow-y-auto whitespace-nowrap'>{props.children}</div>
        </div>
    </>
}
...
const Command: React.FC<CommandInterface> = (props) => {
    ...
    return <CodeBlock copy_text={copy_text}>
        ...
        }) <CodeText type="operator">{"->"}</CodeText> <CodeText type="class">{props.type}</CodeText>
    </CodeBlock>
}
