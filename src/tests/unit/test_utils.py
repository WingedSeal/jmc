...
from jmc.compile.utils import SingleTon, is_connected, is_number
from jmc.compile.command.utils import (
    ArgType,
    PlayerType,
    eval_expr,
    find_arg_type,
...
class TestCommandUtils(unittest.TestCase):
    ...
    def test_find_scoreboard_player_type(self):
        ...
        self.assertEqual(scoreboard_player.value, (DataPack.var_name, "$jmc_var"))

        scoreboard_player = find_scoreboard_player_type(
            Token.empty("OBJ:SELECTOR"), EMPTY_TOKENIZER
        )
        self.assertEqual(scoreboard_player.player_type, PlayerType.SCOREBOARD)
        self.assertEqual(scoreboard_player.value, ("OBJ", "SELECTOR"))

    def test_find_arg_type(self):
        self.assertEqual(
            find_arg_type([Token.empty("5")], EMPTY_TOKENIZER), ArgType.INTEGER
        )
        self.assertEqual(
            find_arg_type([Token.empty("@a")], EMPTY_TOKENIZER), ArgType.SELECTOR
        )
