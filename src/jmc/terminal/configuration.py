...
class Configuration:
    ...
    def load_config(self) -> None:
        ...
        """
        try:
            with (self.global_data.cwd / self.global_data.CONFIG_FILE_NAME).open(
                "r", encoding="utf-8"
            ) as file:
                json = load(file)
    ...
    def save_config(self):
        ...
        """
        pprint(
            f"Your configuration has been saved to {self.global_data.CONFIG_FILE_NAME}",
            Colors.INFO,
        )
        with (self.global_data.cwd / self.global_data.CONFIG_FILE_NAME).open(
            "w", encoding="utf-8"
    ...
    def ask_and_save(self):
        ...
        """
        pprint(
            f"No config file found, generating {self.global_data.CONFIG_FILE_NAME}...",
            Colors.INFO,
        )
        self.ask_config()
        if self.is_configed:
