...
def append_commands(commands: list[str], string: str) -> None:
    ...
    if (
        commands
        and commands[-1] == "run"
        and string.startswith("execute ")
        and commands[-2] != "return"
    ):
...
class FuncContent:
    ...
    def parse_self_command(self, current_line: int, _is_switch: bool = False):
        ...
        if (
            ...
            raise JMCSyntaxException(
                f"Expected keyword (got {self.command[0].token_type.value})",
                self.command[0],
                self.tokenizer,
            )
        ...
        if self.lexer.if_else_box:
            if self.command[0].string != "else":
                append_commands(
                    self.__commands,
                    self.lexer.parse_if_else(self.tokenizer, self.prefix),
                )
            ...
            if self.expanded_commands is not None:
                for expanded_command in self.expanded_commands:
                    self.command_strings.append(
                        ...
                        + (
                            expanded_command
                            if "\n" not in expanded_command
                            else self.lexer.datapack.add_private_function(
                                "expand", expanded_command
                            )
                        )
                    )
            else:
    ...
    def __optimize(self, token: Token) -> bool:
        ...
        if (
            token.string == "@s"
            and self.__commands[-1] == "as"
            and self.__commands[-2] not in {"rotated", "positioned"}
        ):
        ...
        # `execute run`
        if token.string == "run" and self.__commands[-1] == "execute":
    ...
    def __not_expect_command(
        self, key_pos: int, token: Token, command_pos: int
    ) -> None:
        """
        ...
        if token.string == "run" and token.token_type == TokenType.KEYWORD:
            if not self.is_execute and self.__commands[-1] != "return":
                raise MinecraftSyntaxWarning(
                    "'run' keyword found outside 'execute' command",
                    token,
                    self.tokenizer,
                )
            self.is_expect_command = True
        ...
        __is_first_arg = (
            ...
            or __token_string in Header().commands
            or is_decorator(__token_string)
        )
        __is_not_exception = len(self.__commands) > command_pos and not (
            __token_string in FIRST_ARGUMENTS_EXCEPTION
            and self.command[command_pos].string
            in FIRST_ARGUMENTS_EXCEPTION[__token_string]
        )
        ...
        if (
            ...
            and __is_first_arg
            and __is_not_exception
            and __is_not_connected
        ...
        if token.token_type == TokenType.PAREN_ROUND:
            ...
            if (
                ...
                and self.command[key_pos - 1].token_type == TokenType.KEYWORD
            ):
                raise JMCSyntaxException(
                    "Expected whitespace between string and a keyword",
                    self.command[key_pos - 1],
                    self.tokenizer,
                    col_length=True,
                )
            ...
            token.token_type == TokenType.KEYWORD
            and is_connected(token, self.command[key_pos - 1])
            and self.command[key_pos - 1].token_type == TokenType.STRING
        ):
            raise JMCSyntaxException(
                "Expected whitespace between string and a keyword",
                self.command[key_pos - 1],
    ...
    def __expect_command(self, key_pos: int, token: Token, current_line: int) -> bool:
        ...
        if token.token_type != TokenType.KEYWORD and __nbt_type is None:
            ...
            force_create_func = (
                len(self.programs) > current_line + 1
                and self.programs[current_line + 1][0].string == "with"
            )
            append_commands(
                self.__commands,
                self.lexer.datapack.add_arrow_function(
                    "anonymous",
                    token,
                    self.tokenizer,
                    self.prefix,
                    force_create_func=force_create_func,
                ),
            )
        ...
        if (
            ...
            and len(self.command) == key_pos + 2
            and self.command[key_pos + 1].token_type == TokenType.STRING
        ):
            append_commands(
                self.__commands, f"function {self.command[key_pos + 1].string}"
            )
            return SKIP_TO_NEXT_LINE
        ...
        if (
            token.string in {"new", "class", "import"}
            or (
                token.string == "function"
                and len(self.command) == key_pos + 4
    ...
    def __handle_function_call(self, key_pos: int, token: Token) -> bool:
        if len(self.command[key_pos:]) > 2:  # func_name() with ...
            if token.string == "unless":
                raise JMCSyntaxException(
                    ...
                    self.tokenizer,
                    col_length=True,
                    suggestion="Did you mean `if (!...`? ('unless' is not a keyword)",
                )
        ...
        if self.command[key_pos + 1].string != "()":
            ...
            raise JMCSyntaxException(
                f"Unrecognized vanilla macro syntax",
                arg_token,
                self.tokenizer,
                suggestion='Available syntaxes are `func({"key":"value"});`, `func(key="value")`',
            )
    ...
    def __handle_with_anon(self, key_pos: int, token: Token) -> None:
        if not self.was_anonym_func:
            if (
                not self.command_strings
                or "run function" not in self.command_strings[-1]
            ):
                return
    ...
    def __handle_with(self, key_pos: int, token: Token) -> bool:
        ...
        if self.command[key_pos + 1].token_type == TokenType.PAREN_SQUARE and (
            ...
            if len(self.command) > key_pos + 2:
                raise JMCSyntaxException(
                    f"Unexpected token after square parenthesis ({self.command[key_pos + 2].get_full_string()})",
                    self.command[key_pos + 2],
                    self.tokenizer,
                )
            args = verify_list(
                ArgType.SCOREBOARD, self.command[key_pos + 1], self.tokenizer
            )
        ...
        if __with_nbt_type is None:
            ...
            if token_.token_type != TokenType.PAREN_CURLY:
                suggestion = None
                if token_.string == "storage":
                    suggestion = "It appears you want to use 'storage'. Use 'with my_storage::my_path' instead."
                elif token_.string == "entity":
                    suggestion = "It appears you want to use 'entity'. Use 'with @a[...]' instead."
                elif token_.string == "block":
                    suggestion = (
            ...
            if len(self.command) > key_pos + 2:
                raise JMCSyntaxException(
                    f"Unexpected token after curly parenthesis",
                    self.command[key_pos + 2],
                    self.tokenizer,
                )
            if self.switch_tokens is not None:
    ...
    def __handle_say(self, key_pos: int, token: Token) -> None:
        ...
        if self.command[key_pos + 1].token_type != TokenType.STRING:
            raise JMCSyntaxException(
                "Expected string after 'say' command",
                self.command[key_pos + 1],
                self.tokenizer,
                suggestion="(In JMC, you are required to wrapped say command's argument in quote.)",
            )
        ...
        if "\n" in self.command[key_pos + 1].string:
            raise JMCSyntaxException(
                "Unexpected newline in say command",
                self.command[key_pos + 1],
                self.tokenizer,
                suggestion=r"Use '\\n' instead of '\n'",
            )
    ...
    def __handle_schedule(self, key_pos: int) -> bool:
        if len(self.command) < key_pos + 3:
            return CONTINUE_LINE
        if self.command[key_pos + 2].token_type == TokenType.PAREN_CURLY:
        ...
        if (
            ...
            if self.command[key_pos + 1].string == "clear":
                if len(self.command) > key_pos + 4:
                    raise JMCSyntaxException(
                        "Unexpected token in schedule clear",
                        self.command[key_pos + 3],
                        self.tokenizer,
                    )
                return SKIP_TO_NEXT_LINE
            if len(self.command) < key_pos + 5:
                raise JMCSyntaxException(
                    "Expected time in schedule call",
                    self.command[key_pos + 3],
                    self.tokenizer,
                )
            ...
            return SKIP_TO_NEXT_LINE
        if self.command[key_pos + 3].token_type == TokenType.PAREN_CURLY:
            # `schedule 1t append {say "command";}`
            if self.command[key_pos + 2].string not in {"append", "replace"}:
                raise JMCSyntaxException(
                    f"Expected 'append' or 'replace' (got {self.command[key_pos + 2]}",
                    self.command[key_pos + 2],
                    self.tokenizer,
                )
            append_commands(self.__commands, "schedule")
    ...
    def __handle_startswith_var(self, key_pos: int) -> bool:
        ...
        try:
            ...
            try:
                ...
                if getattr(normal_error, "priority", False):
                    raise normal_error
                if (
                    token.string not in VANILLA_COMMANDS
                    and token.string not in Header().commands
    ...
    def __is_jmc_function(self, key_pos: int, token: Token) -> bool:
        ...
        if execute_excluded_command is not None:
            if self.is_execute:
                append_commands(
                    self.__commands,
                    self.lexer.datapack.add_raw_private_function(
                        "anonymous",
                        [
                            execute_excluded_command(
                                self.command[key_pos + 1],
                                self.command[key_pos],
                                self.lexer.datapack,
                                self.tokenizer,
    ...
    def get_function(
        self, token: Token, command_functions: dict[str, type["JMCFunction"]]
    ) -> type["JMCFunction"] | None:
        """
