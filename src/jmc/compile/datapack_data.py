...
class GUI:
    ...
    def get_reset_commands(self) -> list[str]:
        ...
        for index, template_item in enumerate(self.template):
            ...
            commands.append(
                f"execute unless score {template_item.variable} matches 2..{len(template_item.items)} run item replace {self.mode.value[1]} {self.mode.value[0]}.{index} with {matched_item} 1")
            commands.extend(
