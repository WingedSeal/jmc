...
def _hardcode_process(
    string: str, index_string: str, i: str, token: Token, tokenizer: Tokenizer
) -> str:
    ...
    def call(self) -> str:
        ...
        for index in range(len(string_lists[0])):
            try:
                ...
                error.reinit(
                    lambda string: _hardcode_processes(
                        string,
                        index_strings,
                        [string_list[index] for string_list in string_lists],
                        self.token,
...
class RaycastSimple(JMCFunction):
    ...
    def call(self) -> str:
        current_iter = self.current_iter + self.datapack.get_count(
            f"{self.name}/current_iter"
        )
        ...
        if is_stop_block:
            self.datapack.add_private_json(
                "tags/blocks",
                f"{self.name}/default_raycast_pass",
                {
                    "values": [
                        "minecraft:air",
                        "minecraft:void_air",
        ...
        if "overideRecursion" in self.raw_args:
            ...
            modify_execute_before_step = (
                self.args["modifyExecuteBeforeStep"] + " "
                if self.args["modifyExecuteBeforeStep"]
                else ""
            )
            modify_execute_after_step = (
                self.args["modifyExecuteAfterStep"] + " "
                if self.args["modifyExecuteAfterStep"]
                else ""
            )
            recursion_commands = [
        ...
        self.datapack.add_raw_private_function(
            f"{self.name}/loop", loop_commands, count=count
        )

        if (
            "maxIter" not in self.raw_args
            or self.raw_args["maxIter"].arg_type == ArgType.INTEGER
        ):
