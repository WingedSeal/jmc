...
class PredicateLocations(JMCFunction):
    def call(self) -> str:
        ...
        for x in range(int(self.args["xMin"]), int(self.args["xMax"]) + 1):
            for y in range(int(self.args["yMin"]), int(self.args["yMax"]) + 1):
                for z in range(int(self.args["zMin"]), int(self.args["zMax"]) + 1):
                    predicates.append(
                        {
...
@func_property(
    ...
    arg_type={"idName": ArgType.KEYWORD, "functionMap": ArgType.JS_OBJECT},
...
class RightClickSetup(EventMixin):
    ...
    def call(self) -> str:
        ...
        if self.is_never_used():
            self.add_event(
                "used:carrot_on_a_stick", self.datapack.call_func(self.name, "main")
            )
        ...
        if is_switch:
            ...
            for func, is_arrow_func in func_map.values():
                if is_arrow_func:
                    func_contents.append([func])
                else:
                    func_contents.append([f"function {self.datapack.namespace}:{func}"])
...
@func_property(
    ...
    defaults={"displayName": "", "lore": "", "nbt": "", "component": "", "onClick": ""},
)
...
class ItemCreateSpawnEgg(EventMixin, ItemMixin):
    def call(self) -> str:
        ...
        self.add_event(
            "used:" + spawn_egg,
            f"""execute as @e[type=marker,tag=__spawn_egg_{item_id}] at @s run {self.datapack.add_raw_private_function(self.name, ["kill @s", on_place])}""",
        )

        if self.datapack.version >= PackVersionFeature.COMPONENT:
            modify_component = {
                "entity_data": Token.empty(
    ...
    def call(self) -> str:
        ...
        if is_front_glow:
            self.datapack.version.require(
                PackVersionFeature.SIGN_BACK_TEXT,
        ...
        if self.datapack.version >= PackVersionFeature.SIGN_BACK_TEXT:
            if len(texts) > 8:
                raise JMCValueError(
                    ...
                    self.tokenizer,
                    suggestion="4 lines for the front and 4 lines for the back",
                )
        else:
            if len(texts) > 4:
                raise JMCValueError(
        ...
        formatted_texts_ = [
            (
                FormattedText(text, text_token, self.tokenizer, self.datapack)
                if text
                else FormattedText.empty(self.tokenizer, self.datapack)
            )
        ...
        if on_click:
            ...
            formatted_texts_[0].add_key(
                outer_key, {"action": "run_command", inner_key: on_click}
            )
...
@func_property(
    ...
    arg_type={"objective": ArgType.KEYWORD, "triggers": ArgType.JS_OBJECT},
    name="trigger_setup",
    ignore={"triggers"},
)
class TriggerSetup(JMCFunction):
    def call(self) -> str:
        ...
        if self.is_never_used():
            self.datapack.add_tick_command(self.datapack.call_func(self.name, "main"))
            self.make_empty_private_function("main")
        ...
        if is_switch:
            ...
            run = [
                parse_switch(
                    ScoreboardPlayer(PlayerType.SCOREBOARD, (obj, "@s")),
                    func_contents,
                    self.datapack,
                    self.name,
                ),
...
@func_property(
    ...
    arg_type={"objective": ArgType.KEYWORD, "function": ArgType.FUNC},
    name="trigger_add",
)
    ...
    def call(self) -> str:
        ...
        if mode not in {"runOnce", "runTick", "none"}:
            raise JMCSyntaxException(
                f"Avaliable modes for {self.call_string} are 'runOnce', 'runTick' and 'none' (got '{mode}')",
                self.raw_args["mode"].token,
                self.tokenizer,
                suggestion="'runOnce' run the commands once after the timer is over.\n'runTick' run the commands every tick if timer is over.\n'none' do not run any command.",
            )

        if mode in {"runOnce", "runTick"} and ("function" not in self.raw_args):
            raise JMCMissingValueError("function", self.token, self.tokenizer)
        if mode == "none" and "function" in self.raw_args:
            raise JMCSyntaxException(
                f"'function' is provided in 'none' mode {self.call_string}",
...
@func_property(
    ...
    name="recipe_table",
    defaults={"baseItem": "minecraft:knowledge_book", "onCraft": ""},
)
    ...
    def call(self) -> str:
        ...
        if "result" in json:
            if "item" not in json["result"]:
                raise JMCSyntaxException(
                    "'item' key not found in 'result' in recipe",
                    self.raw_args["recipe"].token,
                    self.tokenizer,
                    display_col_length=True,
                    suggestion="recipe json maybe invalid",
                )
            if "count" not in json["result"]:
                raise JMCSyntaxException(
                    "'count' key not found in 'result' in recipe",
                    self.raw_args["recipe"].token,
                    self.tokenizer,
                    display_col_length=True,
                    suggestion="recipe json maybe invalid",
        ...
        self.datapack.add_private_json("recipes", f"{self.name}/{count}", json)
...
class GUITemplate(JMCFunction):
    ...
    def call(self) -> str:
        ...
        self.datapack.data.guis[name] = GUI(name, mode, template)
        if self.is_never_used():
            self.datapack.add_load_command(
        ...
        self.datapack.add_raw_private_function(
            f"gui/{name}",
            [
                f"""execute if entity @p[distance=..8] run {self.datapack.add_raw_private_function(f"gui/{name}", [f"data modify storage {self.datapack.namespace}:{self.datapack.storage_name} GUI.Items set from {mode.value[1]} {mode.value[2]}", f"execute store result score __gui__.item_count {self.datapack.var_name} if data storage {self.datapack.namespace}:{self.datapack.storage_name} GUI.Items[].tag.__gui__", f'execute if score __gui__.item_count {self.datapack.var_name} matches 0 run {self.datapack.call_func(f"gui/{name}", "reset")}'], "active")}""",
                (
                    "execute if block ~ ~-1 ~ hopper run data merge block ~ ~-1 ~ {TransferCooldown:20d}"
                    if mode == GUIMode.BLOCK
                    else ""
                ),
            ],
            "run",
...
class GUIRegisters(ItemMixin):
    def call(self) -> str:
        ...
        item_strings, item_strings_tokens = self.datapack.parse_list(
            self.raw_args["items"].token, self.tokenizer, TokenType.KEYWORD
        )
        if not item_strings:
            raise JMCValueError(
        ...
        if self.args["onClick"]:
            ...
            interactive_id = self.datapack.get_count(f"{self.name}/{name}")
            for item_str, item_str_token in zip(item_strings, item_strings_tokens):
                ...
                items.append(
                    self.clone_item(
                        self.datapack.data.item[item_str],
                        modify_nbt={
                            "__gui__": Token.empty(
                                f"{{interactive_id:{interactive_id},name:{repr(name)}}}"
                            )
                        },
                        error_token=item_str_token,
                    )
                )
            ...
            self.set_items(name, self.args["id"], items, self.raw_args["id"].token)
    ...
    def set_items(
        ...
        for item in items:
            if item.item_type == "air":
                raise JMCValueError("Item cannot be air", token, self.tokenizer)
...
class GUICreate(JMCFunction):
    ...
    def call(self) -> str:
        ...
        compare_player_uuid = self.datapack.call_func("__gui__", "compare_player_uuid")

        container_changed = self.datapack.private_functions[f"gui/{name}"][
            "container_changed"
        ]
        gui = self.datapack.data.guis[name]
        if gui.is_created:
        ...
        for index, template_item in enumerate(gui.template):
            ...
            on_change = self.datapack.add_raw_private_function(
                ...
                [
                    ...
                    (
                        f"""execute as @p[tag=__gui__.clicker] at @s run {template_item.on_click}"""
                        if template_item.interactive_id is not None
                        else ""
                    ),
        ...
        container_changed.extend(
            [
                "kill @e[type=minecraft:item,nbt={Item:{tag:{__gui__:{name:%s}}}}]"
                % repr(name),
    ...
    def call(self) -> str:
        ...
        if self.args["team"] in self.datapack.data.teams:
            ...
            raise JMCValueError(
                ...
                self.tokenizer,
                suggestion=f"It was defined at line {team[1].line} col {team[1].col}",
            )
        ...
        properties = (
            self.tokenizer.parse_js_obj(self.raw_args["properties"].token)
...
class TextPropClickCommand(JMCFunction):
    def call(self) -> str:
        ...
        self.add_formatted_text_prop(
            outer_key,
            {"action": "run_command", inner_key: "/" + command[0]},
...
class TextPropsClickCommand(JMCFunction):
    def call(self) -> str:
        ...
        def inner(arg: str) -> SIMPLE_JSON_BODY:
            return {
                "action": "run_command",
                inner_key: "/" + command[0].replace(self.args["indexString"], arg),
            }

        self.add_formatted_text_prop(outer_key, inner, self.check_bool("local"))
        return ""
...
class TextPropSuggestCommand(JMCFunction):
    def call(self) -> str:
        ...
        self.add_formatted_text_prop(
            outer_key,
            {"action": "suggest_command", inner_key: "/" + command[0]},
            self.check_bool("local"),
        )
        return ""
...
class TextPropClickURL(JMCFunction):
    def call(self) -> str:
        ...
        if not url:
            raise JMCValueError(
                "Unexpected empty URL", self.raw_args["url"].token, self.tokenizer
            )
        ...
        self.add_formatted_text_prop(
            outer_key,
            {"action": "open_url", inner_key: self.args["url"]},
            self.check_bool("local"),
        )
        return ""
...
class TextPropsClickURL(JMCFunction):
    def call(self) -> str:
        ...
        def inner(arg: str) -> SIMPLE_JSON_BODY:
            return {
                "action": "open_url",
                inner_key: self.args["url"].replace(self.args["indexString"], arg),
            }
...
@func_property(
    ...
    defaults={"local": "false"},
    number_type={"page": NumberType.POSITIVE},
)
...
@func_property(
    ...
    arg_type={"propertyName": ArgType.STRING, "local": ArgType.KEYWORD},
...
class TextPropClipboard(JMCFunction):
    def call(self) -> str:
        ...
        self.add_formatted_text_prop(
            "clickEvent",
            {"action": "copy_to_clipboard", "value": self.args["text"]},
            self.check_bool("local"),
        )
        return ""
...
class TextPropsClipboard(JMCFunction):
    def call(self) -> str:
        ...
        self.add_formatted_text_prop("clickEvent", inner, self.check_bool("local"))
        return ""
...
class TextPropHoverText(JMCFunction):
    def call(self) -> str:
        ...
        self.add_formatted_text_prop(
            outer_key,
            {"action": "show_text", inner_key: text_to_show},
            self.check_bool("local"),
        )
        return ""
    ...
    def call(self) -> str:
        ...
        def inner(arg: str) -> SIMPLE_JSON_BODY:
            text_to_show = json.loads(
                self.format_text("text").replace(self.args["indexString"], arg)
            )
...
class TextPropHoverItem(JMCFunction):
    def call(self) -> str:
        ...
        if self.datapack.version < PackVersionFeature.TEXT_COMPONENT:
            key = "hoverEvent"
            body = {"action": "show_item", "contents": item_to_show}
        else:
            key = "hover_event"
            body = {"action": "show_item", **item_to_show}
    ...
    def call(self) -> str:
        ...
        def inner(arg: str) -> SIMPLE_JSON_BODY:
            item_to_show = json.loads(
                self.args["item"].replace(self.args["indexString"], arg)
            )
...
class TextPropHoverEntity(JMCFunction):
    def call(self) -> str:
        ...
        if self.datapack.version < PackVersionFeature.TEXT_COMPONENT:
            key = "hoverEvent"
            body = {"action": "show_entity", "contents": entity_to_show}
        else:
            key = "hover_event"
            body = {"action": "show_entity", **entity_to_show}
    ...
    def call(self) -> str:
        ...
        def inner(arg: str) -> SIMPLE_JSON_BODY:
            entity_to_show = json.loads(
                self.args["entity"].replace(self.args["indexString"], arg)
            )
...
class TextPropsFont(JMCFunction):
    def call(self) -> str:
        ...
        self.add_formatted_text_prop("font", inner, self.check_bool("local"))
        return ""
...
class TextPropsKeybind(JMCFunction):
    def call(self) -> str:
        ...
        self.add_formatted_text_prop("keybind", inner, self.check_bool("local"))
        return ""
...
@func_property(
    ...
    name="text_prop_nbt",
    defaults={"separator": ", ", "interpret": "false", "local": "false"},
)
...
@func_property(
    ...
    arg_type={"variable": ArgType.SCOREBOARD, "src": ArgType.KEYWORD},
    defaults={"src": "true"},
    name="debug_watch",
)
class DebugWatch(JMCFunction):
    ...
    def call(self) -> str:
        ...
        if self.datapack.data.is_too_late_debug_watch:
            raise JMCSyntaxException(
                f"At least 1 variable operation was performed before calling {self.call_string}",
                self.self_token,
                self.tokenizer,
            )
        ...
        if (objective, player) in self.datapack.data.watching:
            raise JMCSyntaxException(
                f"{objective}:{player} is already being watched.",
                self.raw_args["variable"].token,
                self.tokenizer,
            )
    ...
    @classmethod
    def variable_operation_wrapper(
        cls, return_command: str, player: str, objective: str, datapack: "DataPack"
    ) -> str:
        ...
        if objective != datapack.var_name:
            tellraw += (
                f',{{"text":"{objective}","color":"red"}},{{"text":":","color":"aqua"}}'
            )
        tellraw += f',{{"text":"{player} ","color":"gold"}}'
        tellraw += ',{"text":"| ","color":"aqua","bold":true}'
        tellraw += (
            f',{{"score":{{"name":"{cls.tmp}","objective":"{datapack.var_name}"}}}}'
        )
        tellraw += ',{"text":" -> ","color":"aqua","bold":true}'
        ...
        if datapack.data.watching[(player, objective)]:
            tellraw += ',{"text":" | ","color":"aqua","bold":true}'
            tellraw += (
                f',{{"text":"{datapack.data.last_code_data[0]}","color":"yellow"}}'
            )
            tellraw += ',{"text":" | ","color":"aqua","bold":true}'
            tellraw += (
                f',{{"text":"{datapack.data.last_code_data[1]}","color":"yellow"}}'
            )
        tellraw += "]"
        return datapack.add_raw_private_function(
            cls.function_group_name,
            [
                f"scoreboard players operation {cls.tmp} {datapack.var_name} = {player} {objective}",
                return_command,
                f"execute unless score {player} {objective} = {cls.tmp} {datapack.var_name} run "
                + tellraw,
            ],
        )
...
@func_property(
    ...
    arg_type={"variable": ArgType.SCOREBOARD, "cache": ArgType.INTEGER},
    defaults={"cache": "5"},
    number_type={"cache": NumberType.POSITIVE},
    name="debug_history",
)
class DebugHistory(JMCFunction):
    ...
    def call(self) -> str:
        ...
        self.datapack.add_load_command(
            f'scoreboard objectives modify {self.scoreboard_name} displayname {{"text":"History of {obj}:{score}","color":"gold","bold":true}}'
        )
        record_call = self.datapack.add_raw_private_function(
            self.name,
            [
                *(
...
@func_property(
    func_type=FuncType.LOAD_ONLY,
    call_string="Debug.trackFunction",
    arg_type={
        "regex": ArgType.STRING,
        "prefix": ArgType.STRING,
        "suffix": ArgType.STRING,
        "color": ArgType.STRING,
        "functionColor": ArgType.STRING,
    },
    defaults={
        "prefix": "Calling ",
        "suffix": ".",
    ...
    name="debug_track_function",
)
class DebugTrackFunction(JMCFunction):
    ...
    def call(self) -> str:
        try:
            ...
            raise JMCValueError(
                "Invalid regex",
                self.raw_args["regex"].token,
                self.tokenizer,
                suggestion=str(e),
            )
        Header().track_function_regexs.append(
            (
                pattern,
                self.args["prefix"],
                self.args["suffix"],
                self.args["color"],
                self.args["functionColor"],
            )
        )
        return ""
